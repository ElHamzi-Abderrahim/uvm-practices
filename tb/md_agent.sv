...
class md_agent#(int unsigned DATA_WIDTH = 32, type ITEM_DRIVE = md_item_drv) extends uvm_component implements md_reset_handler ;
  ...
  virtual function void build_phase(uvm_phase phase);
	...
	virtual function void connect_phase(uvm_phase phase);
		...
		// Retrieve the vif from the uvm_config_db
	...
	virtual function void handle_reset(uvm_phase phase);
		// Queue of uvm components that will store agent children
		uvm_component children[$] ; 
		// Return to children queue the children of the current agent
